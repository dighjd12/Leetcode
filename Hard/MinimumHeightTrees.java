...
public List<Integer> findMinHeightTrees(int n, int[][] edges) {
    ...
    while(n>2){ //tree is connected, so if n==3, then the middle one is the only root.
        n = n-leaves.size();
 
        LinkedList<Integer> newLeaves = new LinkedList<Integer>();
 
        for(int l: leaves){
            //go through the leaves and merge with the parent node
            //if we find a new leaf, then we should add them to next set of leaves
